...
define(function(require, exports, module) {
	"use strict";
	var CommonTools = require('core/MobileFrame/CommonUtil');

	var HtmlTools = require('core/MobileFrame/HtmlUtil');
	...
	PullDownRefresh.prototype.ajaxRequest = function() {
		...
		mui.ajax(url, {
			...
			//headers 过滤头部信息，判断请求之前是否需要验证规则（比如登录）
	...
	PullDownRefresh.prototype.successRequest = function(response, isInitSessionData) {
		...
		if(self.options.bizlogic.isRendLitemplateAuto) {
			...
			//必须包含Mustache
			if(CommonTools.IsInclude('mustache.min.js')) {
	...
	PullDownRefresh.prototype.defaultChangeResponseData = function(response) {
		var self = this;
		var outData = null;
		if(response != null && response.totalCount && response.data && Array.isArray(response.data)) {
			outData = response.data;
			self.totalCount = response.totalCount || 0;
		} else if(response != null && response.EpointDataBody != null && response.EpointDataBody.DATA != null && response.EpointDataBody.DATA.UserArea != null) {
			//两种情况一种是userArea下面包pageInfo和infolist
			//另一种就是 userArea本身就是数组,pageInfo和他并列
			if(Array.isArray(response.EpointDataBody.DATA.UserArea)) {
				//第一种,userArea本身就是数组
				//设置totalcount
				if(response.EpointDataBody.DATA.PageInfo != null && response.EpointDataBody.DATA.PageInfo.TotalNumCount != null) {
					self.totalCount = response.EpointDataBody.DATA.PageInfo.TotalNumCount;
				} else if(response.EpointDataBody.DATA.UserArea[0] && response.EpointDataBody.DATA.UserArea[0].totalcount) {
					//或者totalcount在userArea里
					self.totalCount = response.EpointDataBody.DATA.UserArea[0].totalcount;
				} else {
					self.totalCount = 0;
				}
				outData = [];
				for(var i = 0; i < response.EpointDataBody.DATA.UserArea.length; i++) {
					outData[i] = response.EpointDataBody.DATA.UserArea[i];
				}
			} else {
				//另一种
				//设置totalcount
				...
				if(response.EpointDataBody.DATA.UserArea.InfoList != null && response.EpointDataBody.DATA.UserArea.InfoList.Info != null) {
					outData = [];
					//服务器端返回了两种格式,一种是JSON数组
					if(Array.isArray(response.EpointDataBody.DATA.UserArea.InfoList.Info)) {
						for(var i = 0; i < response.EpointDataBody.DATA.UserArea.InfoList.Info.length; i++) {
							outData[i] = response.EpointDataBody.DATA.UserArea.InfoList.Info[i];
						}
					} else {
						//一种是JSON对象
						outData[0] = response.EpointDataBody.DATA.UserArea.InfoList.Info;
					}
				}
			}

		} else if(response && Array.isArray(response)) {
			//这种返回response作为全部数组数据
			self.totalCount = 0;
			outData = response;
		} else {
			//默认将数据返回
	...
	exports.initPullDownRefresh = function(options, callback) {
		...
		if(options.skin === 'default') {
			require.async('core/MobileFrame/RayApp.PullToRefresh.Base.Default.js', generatePullToRefreshCallback);
		} else if(options.skin === 'type0') {
			require.async('core/MobileFrame/RayApp.PullToRefresh.Base.Type0.js', generatePullToRefreshCallback);
		} else {
			//其它皮肤都需要引入css
			CommonTools.importFile('css/RayApp/RayApp.PullToRefresh.css');
			if(options.skin === 'type1') {
				require.async('core/MobileFrame/RayApp.PullToRefresh.Base.Type1.js', generatePullToRefreshCallback);
			} else if(options.skin === 'type1_material1') {
				require.async('core/MobileFrame/RayApp.PullToRefresh.Base.Type1.material1.js', generatePullToRefreshCallback);
			} else {
