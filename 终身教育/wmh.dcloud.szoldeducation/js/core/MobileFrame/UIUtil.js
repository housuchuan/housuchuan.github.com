 ...
 * @author hybo dailc
 * @version 2.0
 * @time 2016-01-28 9:03:20
 * 功能模块:
 * UI相关模块********************************
 * core/MobileFrame/UIUtil
 * 1.创建系统选择按钮框
 * 2.创建系统提示对话框
 * 3.创建确认对话框
 * 4.显示系统等待对话框
...
define(function(require, exports, module) {
				...
				plus.nativeUI.actionSheet(config, function(e) {
					if (e.index > 0) {
						if (chooseCallBack && typeof(chooseCallBack) == "function") {
							//回调 
							chooseCallBack(buttons[e.index - 1]);
						}
					}
				});
			}
	...
	function showDtPicter(options, chooseCallBack) {
		options = options || {};
		if (CommonUtil.IsInclude('mui.min.css') &&
			//这个和mui.picker有区别
			CommonUtil.IsInclude('mui.picker.min.css') &&
			CommonUtil.IsInclude('mui.min.js') &&
			//这个和mui.picker有区别
			CommonUtil.IsInclude('mui.picker.min.js')) {
	...
	var popPicker3=null;
	var popPicker2=null;
	exports.showPopPicker = function(data, chooseCallBack, num) {
		if (CommonUtil.IsInclude('mui.min.css') &&
			CommonUtil.IsInclude('mui.picker.min.css') &&
			CommonUtil.IsInclude('mui.poppicker.css') &&
			CommonUtil.IsInclude('mui.min.js') &&
			CommonUtil.IsInclude('mui.picker.min.js') &&
			CommonUtil.IsInclude('mui.poppicker.js')) {
				num=num||nums;
			if (num=='3') {
					var popPicker3 = popPicker3 || new mui.PopPicker({
						layer: num
					});
					data = data || [];
					popPicker3.setData(data);
					popPicker3.show(function(items) {
						if (chooseCallBack && typeof(chooseCallBack) == 'function') {
							chooseCallBack(items[0].text+';'+items[1].text+';'+items[2].text+';', items[0].value+';'+items[1].value+';'+items[2].value+';', items[0]);
						}
					});
				}else if(num=='2'){
					var popPicker2 = popPicker2 || new mui.PopPicker({
						layer: num
					});
					data = data || [];
					popPicker2.setData(data);
					popPicker2.show(function(items) {
						if (chooseCallBack && typeof(chooseCallBack) == 'function') {
							chooseCallBack(items[0].text+';'+items[1].text+';', items[0].value+';'+items[1].value+';', items[0]);
						}
					});
				}else if(num==null){
	...
	/**
	 * 
	 * @param {Object} realwidth内容之际宽度
	 */
	exports.tabbar = function(realwidth, leftwidth) {
		leftwidth = leftwidth || 0;
		console.log(leftwidth);
		Zepto('.tab_silder_view').css({
			'-webkit-transform': 'translate3d(' + leftwidth + 'px, 0px, 0px)'
		});
		Zepto('.tab_silder_view').css({
			'transform': 'translate3d(' + leftwidth + 'px, 0px, 0px)'
		});
		var xclient, xclientmove, xdif, xtotal = 0,
			maxwidth;
		var winwidth = '';
		if (CommonUtil.IsInclude('zepto.min.js')) {
			Zepto('.tab_silder_view').bind('touchstart', function(e) {
				var touch = e.touches[0];
				xclient = touch.pageX;
				winwidth = document.body.clientWidth;
				//console.log(winwidth);
				maxwidth = realwidth - winwidth
					//console.log(xclient);
					//console.log(maxwidth);
			});
			Zepto('.tab_silder_view').bind('touchmove', function(e) {
				if (maxwidth > 0) {
					var touch = e.touches[0];
					xclientmove = touch.pageX;
					//console.log(xclientmove);
					xdif = xclientmove - xclient;
					xclient = xclientmove;
					xtotal = xtotal + xdif;
					if (xtotal > leftwidth) {
						xtotal = leftwidth;
					...
					//console.log(xtotal);
					//Zepto(this).css({'left':xtotal+'px'});
					Zepto(this).css({
						'-webkit-transform': 'translate3d(' + xtotal + 'px, 0px, 0px)'
					});
					Zepto(this).css({
						'transform': 'translate3d(' + xtotal + 'px, 0px, 0px)'
					});
				}
			});
		} else {
