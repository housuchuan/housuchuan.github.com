 ...
 * @author dailc  dailc 
 * @version 2.0
 * @time 2016-01-26 10:20:57
 * 功能模块:
 * Update工具类****************************************
 * core/MobileFrame/UpdateUtil
...
define(function(require, exports, module) {
	...
	var FileUtil = require('core/MobileFrame/FileUtil');
	var DownloadUtil = require('core/MobileFrame/DownLoadUtil');
	var NotificationUtil = require('core/MobileFrame/NotificationUtil');
	...
	function getUpdateJsonFromServer(callbackPool) {
		...
		//请求前先删除本地文件
		FileUtil.delFile(updateFilePath);
		//通过downloadUtil实现,不使用本地缓存
		var tmpOption = defaultSetting;
		tmpOption.getRelativePathFromLoadUrlCallback = function() {
			//获得相对路径的函数
			return updateFilePath;
		};
		DownloadUtil.setOptions(tmpOption);
		DownloadUtil.downloadFileWidthLocalCache(defaultSetting['UpdateUrl'], {
	...
	function checkUpdateData(inf, callbackPool) {
		...
		if (inf && inf[plus.os.name] && inf[plus.os.name].ResourcePackage) {
			// 判断App版本是否需要升级
			if (CommonUtil.compareVersion(exports.getCurrentAppVersion(), inf[plus.os.name].AppVersion) == true) {
				tipChooseUpdate(inf, true, callbackPool);
			} else if (CommonUtil.compareVersion(currentResourceVersion, inf[plus.os.name].ResourcePackage.resourceVersion) == true
	...
	function downWgt(inf, isAppUpdate, callbackPool) {
		...
		DownloadUtil.RestoreOptions();
		DownloadUtil.setOptions(defaultSetting);
		UpDatePackageUrl = upUrl;
		DownloadUtil.downloadFileWidthLocalCache(UpDatePackageUrl, {
			beforeDownload: function() {
	...
	exports.abortUpdate = function() {
		if (UpDatePackageUrl) {
			//如果更新包地址存在
			DownloadUtil.abortTaskByUrl(UpDatePackageUrl);
		}
		//停止更新文件的下载
		DownloadUtil.abortTaskByUrl(defaultSetting['UpdateUrl']);
	};
	/**
	 * @description 初始化更新
		...
		var callbackPool = {
			...
			beforeDownloadJson: function() {
				if (UpdateType == 1) {
					...
					NotificationControl = NotificationUtil.initNotificationControl();
					NotificationControl.setNotification('检查更新', '正在检查更新信息', '更新提示');
					if (plus.os.name == 'iOS') {
						plus.nativeUI.toast('正在检查更新信息');
					}
				} else {
