	...
	function initPullrefresh() {
		var getLitemplate = function() {
			...
			if(type == 0) {
				...
				litemplate = '<li id="{{id}}" class="mui-table-view-cell zhiboItem"><span class="activityId" id="{{activityId}}"></span><div class="imgBorder"><img src="{{image}}"/></div><div class="infoDetailBorder"><div class="zhiboTitle mui-ellipsis">{{title}}</div><div class="introduction">简介：{{introduction}}</div></div></li>';
			}
			return litemplate;
		};
			...
			if(response.code == 1) {
				...
				if(type == 0) {
					if(tempArray) {
						if(Array.isArray(tempArray)) {
							mui.each(tempArray, function(key, value) {
								console.log("xxxxxxxxxxxxx" + JSON.stringify(value));
								if(!value.image) {
