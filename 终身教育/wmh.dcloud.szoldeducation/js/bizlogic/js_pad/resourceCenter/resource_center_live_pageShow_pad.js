...
define(function(require, exports, module) {
	...
	//tab切换
	Zepto('.tabChoose').on('tap','span',function(){
		Zepto(this).css('color','#0000FF').css('border-bottom','1px solid #0000FF').siblings().css('color','#000000').css('border-bottom','none');
		var _this = Zepto(this).text();
		if (_this == '简介') {
			Zepto('.list-container-selection').hide();
			Zepto('p').show();
		}else{
			Zepto('p').hide();
			Zepto('.list-container-selection').show();
		}
	});
	...
	function videoPlaySelections() {
		...
		CommonUtil.ajax(url, requestData, function(response) {
			...
			if(response.code == 1) {
				...
				Zepto('.list-container-selection').append(output);
				//console.log(Zepto('.selections-items').html());
				var selectionsFirId = Zepto('.list-container-selection li:first-child').find(".activityId").attr('id');
				//console.log("xxxxxxxxxxxxxxxxx"+selectionsFirId);
				initLeBroadcast(selectionsFirId);
				//选集点击操作样式变化
				Zepto('.list-container-selection').on('tap', 'li', function() {
	...
	function getBroadCastIntro() {
		...
		CommonUtil.ajax(url, requestData, function(response) {
			...
			if(response.code == 1) {
				var tempInfo = response.data;
				Zepto('p').text(tempInfo.introduction);
			}
			//'1'表示ajax传输类型为1，secretKey表示用户信息，true表示是mock数据
		}, function() {
			UIUtil.toast('网络连接超时！请检查网络...');
