...
define(function(require, exports, module) {
	...
	/*
	 * @description 初始化下拉刷新控件,对整体布局存在一定影响
	 */
				...
				if(userSession.userId) {
					if(userSession.userId === userId) {
						UIUtil.confirm({
							...
							if(index == 0) {
								//f(device.tablet()){
								//WindowUtil.firePageEvent('szpark_personal_space_list_pad.html', 'refreshListPage');
								//}else{
	...
	function initPullRefreshList2() {
		console.log("是事实上事实上事实上事实上事实上事实上");
		...
		//某一些接口是要求参数为字符串的 
		requestData = JSON.stringify(requestData);
		//console.log("列表xxxxxxxxxxxxxxx" + requestData);
		...
		CommonUtil.ajax(url, requestData, function(response) {
			...
			if(response.code == 1) {
				...
				var _this = Zepto('#listdata').find('li');
	...
	//点击侧边栏首页刷新数据
	window.addEventListener('refreshMainPage', function() {
		initPullRefreshList2();
	});

	//点击删除刷新数据
	window.addEventListener('refreshListMainPage', function() {
		initPullRefreshList2();
	})
