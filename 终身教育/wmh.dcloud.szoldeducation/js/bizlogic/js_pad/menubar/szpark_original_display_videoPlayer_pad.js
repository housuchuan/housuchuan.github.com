...
define(function(require, exports, module) {
	...
	function initData() {
		...
		console.error("xxxxxxxxxxxxxxxxxxx" + uuId + 'XXXXXXXXXXXXXXXXXXXXXX' + vuId);
	...
	function initPullRefreshCommentList() {
		...
		var changeResponseDataFunc = function(response) {
			...
			if(response.code == 1) {
				console.error("response00000000000000000000000000000000000000000000000水水水水" + JSON.stringify(response));
			...
			if(Zepto(e.target).hasClass('zanBorder') || Zepto(e.target).parent().hasClass('zanBorder')) {
				...
				if(LoginUtil.isLoginSystem()) {
					...
					UIUtil.confirm({
						...
						if(index == 0) {
							LoginUtil.ResetTARGET_URL('szpark_original_display_videoPlayer_pad.html');
		...
		function ajaxPraiseSubmit(commentId, type) {
			...
			requestData = data;
			requestData = JSON.stringify(requestData);
			console.error("xxxxxxxxxxxxxxxxxxxxxxxxxx" + requestData);
	...
	//监听自定义事件
	window.addEventListener('refreshVideoCommentsListPad', function() {
		//重定向之后刷新评论列表，获取基础信息
		secretKey = StorageUtil.getStorageItem("secretKey");
		var userSession = StorageUtil.getStorageItem("UserSession");
		if(userSession) {
