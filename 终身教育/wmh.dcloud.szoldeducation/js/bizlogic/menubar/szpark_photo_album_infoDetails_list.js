...
define(function(require, exports, module) {
	...
	//监听父页面点击事件
	window.addEventListener('refreshListPage1', function(e) {
		console.log("刷新相册的数目：" + e.detail.photoAlbumInfo);
		var photoAlbumInfoDataBody = JSON.parse(e.detail.photoAlbumInfo) || {};
		if(photoAlbumInfoDataBody.photoAlbumId && photoAlbumInfoDataBody.photoAlbumPhotoNum && photoAlbumInfoDataBody.albumName) {
			//相册ID
			photoAlbumId = photoAlbumInfoDataBody.photoAlbumId.toString();
			//相册里照片数
			photoAlbumPhotoNum = photoAlbumInfoDataBody.photoAlbumPhotoNum.toString();
			//相册名称
			albumName = photoAlbumInfoDataBody.albumName.toString();
			Zepto('.photoNum').text(photoAlbumPhotoNum + '张');
			Zepto('#photoAlbumName').text(albumName);
		};
		secretKey = StorageUtil.getStorageItem("secretKey");
		//		console.log("xxxxxxxxxxxsecretKey"+secretKey);
		initPullRefresh(photoAlbumId);
		//刷新列表
		//WindowUtil.firePageEvent("szpark_photo_album_infoDetails_list.html", "refreshListPage");
