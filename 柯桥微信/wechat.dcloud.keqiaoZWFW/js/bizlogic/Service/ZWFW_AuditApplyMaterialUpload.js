...
define(function(require, exports, module) {
				...
				if(MaterialList.length < 1) {
					...
					document.getElementById('iteminfo').addEventListener('tap', function(e) {
						if(e.target.tagName === 'A') {
							...
							mui.openWindow({
								url: "ZWFW_MaterialAttachUpload.html?ProjectGuid=" + ProjectGuid + "&Mname=" + encodeURIComponent(pars[0]) + "&ClientGuid=" + pars[1] + "&UserPK=" + OpenID + "&maxcount=" + maxcount
							});
						}
	...
	function pushmaterial() {
		var url = config.serverUrl + "/AuditProject/SubmitProject";
		var requestData = {
			ValidateData: Token,
			paras: {
				ProjectGuid: ProjectGuid
