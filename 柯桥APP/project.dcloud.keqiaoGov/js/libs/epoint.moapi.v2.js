...
(function(exports, isLocal) {
	...
	ejs.nativeUI = {
		...
		pullToRefresh: {
			 ...
			 */
			disable: function(){
				JSBridge.callHandler(EJS_API, 'setSwipeRefreshEnable', {'swipeRefreshEnable':false}, function(res) {
			...
			enable: function(callback){
				...
				responseCallbacksLongTerm[callbackId] = function(res){
					callback&&callback(res.result, res.msg, res);
				};
				
				JSBridge.callHandler(EJS_API, 'setSwipeRefreshEnable', {'swipeRefreshEnable':true}, callbackId);
			},
			/**
	...
	ejs.navigator = {
		...
		/**
		 * @description 设置导航栏右上角文字
		 * 需要设置的按钮文字,若为空则隐藏按钮。页面需要实现onNBRight()函数来接收点击事件。
		 * @param {String} title
		 * @param {Function} callback 回调函数,这个回调函数仅仅通知是否调用成功
		 */
